...
export async function gerarTresCopies(input: {
  ...
  const buildPrompt = (angulo: string): string => {
    ...
    `- CTA OBRIGATÓRIO: o ÚLTIMO slide é SEMPRE um CTA com chamada para ação EXPLÍCITA — nunca termine o carrossel só com frase memorável/reflexão. Marque entre [[ ]] APENAS a AÇÃO curta — 2 a 5 palavras ("[[Salva esse post]]", "[[Manda no grupo]]", "[[Comenta EU]]"), NUNCA uma frase inteira; o resto do texto fica fora do [[ ]]. Faça-a CONTINUAÇÃO natural da emoção, nunca "curta e siga". Escolha pela emoção: indignação → opinião forte/dilema; identificação → experiência pessoal; tensão → debate; aprendizado → salvar; indireta → compartilhar; desejo → aspiracional. EVITE repetir "SIM ou NÃO", "Concorda?", "O que acha?".\n` +
