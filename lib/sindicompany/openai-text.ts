...
export async function gerarTresCopies(input: {
  ...
  const buildPrompt = (angulo: string): string => {
    ...
    `- CTA DE POSICIONAMENTO (fórmula campeã — vale pra TODO formato): o ÚLTIMO slide SEMPRE força o leitor a TOMAR UM LADO e comentar — é o que mais viraliza. Dê DUAS opções concretas e carregadas (nunca genéricas) e peça o voto: binário ("[[Comenta SIM ou NÃO]]") ou de lado ("Você teria votado com os 12 ou os 8? [[Comenta 12 ou 8]]" + 1 linha curta legendando cada lado: "12 = síndico profissional · 8 = ficava com o Carlos"). Marque entre [[ ]] APENAS a AÇÃO (2 a 5 palavras), nunca uma frase inteira. NUNCA termine só com frase memorável/reflexão. Só em post claramente utilitário (modelo/script, checklist) vale trocar por "[[Salva esse post]]"/"[[Manda no grupo]]" — mas o DEFAULT é escolher um lado. PROIBIDO o CTA preguiçoso "Concorda?"/"O que acha?" SEM dois lados concretos.\n` +
