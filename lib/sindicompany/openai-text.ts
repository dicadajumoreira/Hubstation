...
export async function gerarTresCopies(input: {
  ...
  const buildPrompt = (angulo: string): string => {
    ...
    `- ÂNCORAS DE ATENÇÃO (retenção): nos slides de conteúdo, marque com [[ ]] dentro do BODY no máximo 1 trecho de alto impacto por slide (no máximo +1 secundário) — verdade incômoda, frase de identificação/tensão, frase memorável (citação) OU palavra-chave forte (ex: "refém", "caos", "processo", "autoridade", "improviso", "conflito", "silêncio", "pressão"). NUNCA mais de 2 por slide; destacar demais tira a força. Prefira deixar slides sem âncora a poluir.\n` +
    `- CTA: no último slide, marque a ação principal entre [[ ]] (ex: "[[Salva esse post]] pra usar depois.") pra ela parecer acionável.\n` +
    `- Cada slide interno: tipo + título (3-7 palavras) + body (1-3 frases curtas, máx 35 palavras). Seja conciso — não encha linguiça.\n` +
    `- MICRO-TENSÃO (retenção entre slides): os slides do MEIO devem terminar com um gancho que puxa o próximo — curiosidade, tensão ou continuação emocional —, nunca 100% fechados. Ex: "Mas o problema começa depois.", "E é aqui que a gestão perde autoridade.", "Só que quase ninguém percebe isso.", "Mas o pior ainda vem.". O usuário tem que pensar "deixa eu ver o próximo". Evite explicação técnica contínua e slides conclusivos no meio. Só o ÚLTIMO slide (CTA) e a frase final memorável podem fechar.\n` +
