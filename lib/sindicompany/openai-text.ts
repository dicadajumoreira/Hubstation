...
export async function gerarTresCopies(input: {
  ...
  const buildPrompt = (angulo: string): string => {
    ...
    `- CTA (último slide): marque a ação entre [[ ]] e faça-a parecer continuação natural da emoção, nunca "curta e siga". Tipos: identificação ("Comenta [[EU]] se já viveu isso."), indireta ("[[Compartilha no grupo]] do condomínio."), salvamento ("[[Salva]] pra lembrar disso."), autoridade ("Gestão premium começa nos [[detalhes]]."), desejo ("Seu condomínio [[merece isso]].").\n` +
