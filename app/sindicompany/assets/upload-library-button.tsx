...
import {
  uploadEmbeddedSindicompanyAssets,
  type UploadResult,
} from "./upload-library-action";

/** Botão "Subir biblioteca embutida" para a marca Sindicompany.
 *  Espelha o UploadLibraryButton do Consvicta — chama a server action
 *  que sobe os PNGs do Brand Hub novo (logos + masks) pros buckets
 *  __logos do Supabase usando o service_role já configurado no Netlify.
 *  Mostra resultado inline. Idempotente. */
export function UploadLibraryButtonSindicompany() {
  ...
  const run = () => {
    ...
    startTransition(async () => {
      const r = await uploadEmbeddedSindicompanyAssets();
      setResult(r);
      setConfirming(false);
      router.refresh();
    });
